...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/admin/product-subtypes?category_id=${catId}`, {
          cache: "no-store",
        });
        const data = await res.json();
        if (res.ok) setProductSubcategories(Array.isArray(data) ? data : []);
        else setProductSubcategories([]);
      } catch {
        setProductSubcategories([]);
      }
    })();
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <section className="rounded-2xl border border-gray-3 bg-white p-6 space-y-4">
        ...
        <div className="block">
          ...
          <QuickLinkHtmlEditor
            editorKey={`${id}-short-description`}
            height={220}
            value={form.short_description ?? ""}
            onChange={(html) => setForm((f: any) => ({ ...f, short_description: html }))}
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <SelectWithCreate
            ...
            onDeleted={(id) => {
              ...
              setProductSubcategories([]);
              setForm((f: any) => ({ ...f, category_id: "", subtype_id: "" }));
            }}
