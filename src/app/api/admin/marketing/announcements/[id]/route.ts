    ...
    async () => {
      ...
      after(() => {
        try {
          ...
          console.error("[announcements PATCH] revalidate failed", err);
        }
      });

      return NextResponse.json({ ok: true }, { status: 200 });
    },
    { name: "PATCH /api/admin/marketing/announcements/[id]" }
  );
}
    ...
    async () => {
      ...
      after(() => {
        try {
          ...
          console.error("[announcements DELETE] revalidate failed", err);
        }
      });

      return NextResponse.json({ ok: true }, { status: 200 });
    },
    { name: "DELETE /api/admin/marketing/announcements/[id]" }
  );
}
