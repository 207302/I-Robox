...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  return runApiRoute(async () => {
    try {
      ...
      await rateLimitStrict(`admin_marketing_br_patch:${req.ip ?? "unknown"}`, 1);
    } catch (e: unknown) {
    ...
    const brand_id =
      body.brand_id !== undefined ? String(body.brand_id) : undefined;
    if (brand_id !== undefined && !isUuid(brand_id)) {
    ...
    await prisma.homepage_brand_rail.update({
      where: { id },
      data: {
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  return runApiRoute(async () => {
    try {
      assertSameOrigin(req);
      await rateLimitStrict(`admin_marketing_br_del:${req.ip ?? "unknown"}`, 1);
    } catch (e: unknown) {
      if (e instanceof Error && e.message === "BAD_ORIGIN") {
        return NextResponse.json({ error: "Bad origin" }, { status: 403 });
      }
    ...
    if (derived?.startsWith("irobox/homepage-brand-rail/")) {
