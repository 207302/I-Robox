...
export async function GET() {
  return runApiRoute(async () => {
    ...
    const rows = await prisma.homepage_brand_rail.findMany({
...
export async function POST(req: NextRequest) {
  return runApiRoute(async () => {
    try {
      ...
      await rateLimitStrict(`admin_marketing_br_post:${req.ip ?? "unknown"}`, 1);
    } catch (e: unknown) {
    ...
    const brand_id = body.brand_id ? String(body.brand_id) : "";
    if (!isUuid(brand_id)) {
    ...
    const created = await prisma.homepage_brand_rail.create({
      data: {
        brand_id,
        image_url: imageUrl,
