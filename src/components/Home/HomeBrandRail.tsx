...
function toHighlightItem(item: HomeBrandRailItem): HighlightCardItem {
  return {
    id: item.id,
    href: item.href,
    image: item.image,
...
type HomeBrandRailProps = {
  items: HomeBrandRailItem[];
};

export default function HomeBrandRail({ items }: HomeBrandRailProps) {
      ...
      <div id={RAIL_ID} className={HOME_RAIL_SCROLL}>
        {items.map((item) => (
          <div key={item.id} className={`${HOME_HIGHLIGHTS_CARD_WIDTH} snap-start`}>
            <HighlightCard item={toHighlightItem(item)} />
          </div>
        ))}
      </div>
