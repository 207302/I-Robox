...
const STORAGE_KEY = "irobox_launch_notify_done";
...
function FieldIcon({ children }: { children: React.ReactNode }) {
  return (
    <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-meta-4">
      {children}
    </span>
  );
}
...
export default function LaunchNotifyPopup() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setPhase("open"), OPEN_DELAY_MS);
  ...
  if (phase === "minimized") {
    return (
      <button
        ...
        className="fixed bottom-24 right-4 z-[61] inline-flex items-center gap-2 rounded-full border border-blue/20 bg-white px-4 py-2.5 text-sm font-semibold text-blue shadow-lg hover:bg-blue/5 sm:right-6"
  ...
  return (
    <div
      className="fixed bottom-24 right-4 z-[61] w-[min(100vw-2rem,340px)] transition-all duration-300 sm:right-6"
      role="dialog"
      aria-modal="false"
        ...
        <form onSubmit={handleSubmit} className="space-y-3 px-4 py-4">
          <label className="relative block">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-3 bg-gray-1 py-2.5 pl-10 pr-3 text-sm outline-none focus:border-blue"
              autoComplete="name"
            />
          </label>
