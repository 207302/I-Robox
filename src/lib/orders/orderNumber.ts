...
/** First customer-facing order number (IRx-10001). */
...
export function formatSequentialOrderNumber(sequenceValue: number): string {
  ...
  return `${ORDER_NUMBER_PREFIX}-${Math.trunc(sequenceValue)}`;
}
...
export function formatOrderReference(order: OrderReferenceSource): string {
  ...
  if (stored) {
    ...
    if (n !== null) return `${ORDER_NUMBER_PREFIX}-${String(n).padStart(5, "0")}`;
    return stored;
  }
  return `${ORDER_NUMBER_PREFIX}-?????`;
}

/** Admin display: IRx- + 5-digit id (e.g. IRX-10046 → IRx-10046). */
...
/** Atomically allocate the next sequential order number (IRx-10001, IRx-10002, …). */
